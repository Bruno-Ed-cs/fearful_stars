...
void Player::update(double dt, Engine::Systems &sys) {
  ...
  if (sys.projectile->check_collisions(graze_range.get(pos.vec()), true)
          .collided &&
      graze_cooldown.past_limit()) {
  ...
  auto collisions_proj_foe =
      sys.projectile->check_collisions(hitbox.get(pos.vec()), true);
  auto collisions_proj =
      sys.projectile->check_collisions(hitbox.get(pos.vec()), false);
  auto collisions_enemy = sys.enemy->check_collisions(hitbox.get(pos.vec()));

  if (collisions_proj_foe.collided || collisions_enemy.has_collided) {
  ...
  // std::cout << position.get_round().x << " " << position.get_round().y << " "
...
void Player::die(Engine::Systems &sys) { dead = true; }
...
void Player::draw() {
  ...
  if (invincible) {
    Engine::RenderMan::send_texture(Engine::RenderMan::Plane::middle,
                                    *spritesheet, dest, origin, 0, 0, RED);

  } else
    Engine::RenderMan::send_texture(Engine::RenderMan::Plane::middle,
                                    *spritesheet, dest, origin);
}
